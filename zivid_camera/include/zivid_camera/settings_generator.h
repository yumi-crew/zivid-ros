...
{
  path = boost::replace_all_copy<std::string>(path, "/", ".");
...
public:
  ...
  Generator(rclcpp_lifecycle::LifecycleNode* lifecycle_node, const ZividSettings& settings,
            const std::string& config_class_name)
    : lifecycle_node_(lifecycle_node), zivid_settings_class_name_(settings.name), config_class_name_(config_class_name)
  {
  }
  ...
  void apply(const ZividSettingNode& s)
  {
      const auto parameter = convertSettingsPathToConfigPath(s.path);
      rcl_interfaces::msg::ParameterDescriptor parameter_descriptor;
      parameter_descriptor.name = s.name;
      parameter_descriptor.description = s.description;
      lifecycle_node_->declare_parameter(parameter, s.value());
  }
...
private:
  rclcpp_lifecycle::LifecycleNode* lifecycle_node_;
  std::string zivid_settings_class_name_;
  std::string config_class_name_;
};
